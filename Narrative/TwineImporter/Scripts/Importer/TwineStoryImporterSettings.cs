...
namespace Celeste.Narrative.TwineImporter
{
    ...
    public class TwineStoryImporterSettings : ScriptableObject
    {
        ...
        {
            foreach (string key in Twine.Tokens.Get(text, locaTokenStartDelimiter, locaTokenEndDelimiter))
            {
        ...
        {
            foreach (string key in Twine.Tokens.Get(conditionText, conditionStartDelimiter, conditionEndDelimiter))
            {
