...
{
    [CreateAssetMenu(fileName = "TryCreateSetBackgroundNode", menuName = "Celeste/Narrative/Twine/Parser Steps/Try Create Set Background Node")]
    public class TryCreateSetBackgroundNode : TwineNodeParserStep
    {
        public override bool CanParse(TwineNodeParseContext parseContext)
        {
            ...
            if (!parseContext.ImporterSettings.IsSetBackgroundInstruction(splitText[0]))
            {
                return false;
            }

            return parseContext.ImporterSettings.IsRegisteredBackgroundKey(splitText[1]);
        }
