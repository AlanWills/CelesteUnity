...
namespace UnityEngine.UI.Extensions
{
    public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollSnap, IPointerClickHandler
    {
        ...
        public void OnScrollToChildWithID(ID id)
        {
            foreach (RectTransform child in _screensContainer)
            {
                IDHolder idHolder = child.GetComponent<IDHolder>();

                if (idHolder != null && idHolder.HasID(id))
                {
                    GoToScreen(child.GetSiblingIndex());
                    break;
                }
            }
        }
